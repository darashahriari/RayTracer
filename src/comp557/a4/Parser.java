...
public class Parser {
	...
	public static Light createLight(Node dataNode) {
		...
		Node sizeAttr = dataNode.getAttributes().getNamedItem("size");
		if (sizeAttr != null) {
			light.size = (int) Double.parseDouble(sizeAttr.getNodeValue());
		}
		light.testType();
		return light;
	}
	...
	public static Material createMaterial(Node dataNode) {
		...
		if (refAttr != null) {
			...
			Node glazedAttr = dataNode.getAttributes().getNamedItem("glazed");
			if (glazedAttr != null) {
				Scanner s = new Scanner(glazedAttr.getNodeValue());
				boolean glazed = s.nextBoolean();
				material.glazed = glazed;
				s.close();
			}
		}
		return material;
	...
	public static Render createRender(Node dataNode) {
		...
		if (jitterAttr != null) {
			...
			System.out.println(render.jitter);
			s.close();
		}
		NodeList nodeList = dataNode.getChildNodes();
		for (int i = 0; i < nodeList.getLength(); i++) {
