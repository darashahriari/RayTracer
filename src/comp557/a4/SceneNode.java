 ...
 * elsewhere in the hierarchy (provided it does not make loops.
 * 
 * Note that if the material (inherited from Intersectable) for a scene node is
 * non-null, it should override the material of any child.
 * 
 */
public class SceneNode extends Intersectable {
	...
	/**
	 * Default constructor. Note that all nodes must have a unique name, so that
	 * they can used as an instance later on.
	 */
	public SceneNode() {
		super();
		this.name = "";
	...
	public void intersect(Ray ray, IntersectResult result, boolean shadow) {
		...
		for (Intersectable child : children) {
			...
			child.intersect(ray, closer, shadow);

			Matrix4d T = new Matrix4d();
			T.transpose(Minv);
			T.transform(closer.n);

			// We only care about this intersection point if it is the closest from light
			// source
